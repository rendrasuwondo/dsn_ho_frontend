...
export default {
  ...
  async mounted() {
    this.recordMenuLog('Report Sortasi Harian PH') // Catat akses menu
    ...
    async loadDropdownData() {
      try {
        const [pksRes, estateRes, afdRes] = await Promise.all([
          this.$axios.$get(`/api/admin/spot-cek-get-pks-by-user`),
          this.$axios.$get(`/api/admin/spot-cek-list-estate-report-by-company`),
          this.$axios.$get(
