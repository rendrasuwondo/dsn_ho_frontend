...
export default {
  ...
  async mounted() {
    this.recordMenuLog('Report Sortasi Harian HO') // Catat akses menu
  ...
  methods: {
    ...
    async loadDropdownData() {
      try {
        const [pksRes, estateRes, afdRes] = await Promise.all([
          this.$axios.$get(`/api/admin/spot-cek-get_pks_dropdown-report`),
          this.$axios.$get(`/api/admin/spot-cek-list-estate-report`),
          this.$axios.$get(`/api/admin/spot-cek-list-afdeling-report`),
        ])
    ...
    async exportToPNG() {
      try {
        ...
        link.href = image
        link.download = `ReportSortasiHarianHO_${this.selectedDate}.png`
