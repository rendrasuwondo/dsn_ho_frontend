...
export default {
  ...
  mounted() {
    ...
    this.field.UPDATE_BY =
      this.$auth.user.employee.nik + '-' + this.$auth.user.employee.name
    // this.field.DOC_STATUS = { CODE: '0', NAME: 'INITIAL' }

    if (this.field.DOC_STATUS === '') {
      this.$axios
        .get('/api/admin/lov_status_workflow?code=0')

        .then((response) => {
          // console.log('daaaa')
          // console.log(response.data.data[0])
          this.field.DOC_STATUS = response.data.data[0]
        })
    }
  ...
  methods: {
    ...
    async storePost() {
      ...
      formData.append(
        'DOC_STATUS',
        this.field.DOC_STATUS ? this.field.DOC_STATUS.CODE : ''
      )
