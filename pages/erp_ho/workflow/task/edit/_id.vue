    ...
    <section class="content" v-if="show === 1">
      <div class="card card-outline card-info">
        <div class="card-header">
          <h3 class="card-title">
            <i class="nav-icon fas fa-clipboard-list"></i> <b>EDIT TAKS</b>
          </h3>
...
export default {
  ...
  mounted() {
    ...
    this.$axios
      .get(`/api/admin/task_workflow/${this.$route.params.id}`)
      .then((response) => {
        console.log('daaa')
        console.log(response.data.data)
        //data yang diambil
        this.field.P_WF_PROC_ID = response.data.data.process
        this.field.P_WF_DOC_TYPE_ID = response.data.data.P_WF_DOC_TYPE_ID
        this.field.TASK = response.data.data.TASK
        this.field.LISTING_NO = response.data.data.LISTING_NO
        this.field.DOC_STATUS = response.data.data.status
        this.field.IS_YES_NO = response.data.data.IS_YES_NO
  ...
  methods: {
    ...
    async update(e) {
      ...
      await this.$axios
        .put(`/api/admin/task_workflow/${this.$route.params.id}`, {
          //data yang dikirim
          IS_ACTIVE: this.field.IS_ACTIVE,
          P_WF_DOC_TYPE_ID: this.field.P_WF_DOC_TYPE_ID
          ...
          P_WF_PROC_ID: this.field.P_WF_PROC_ID.P_WF_PROC_ID
            ...
            : '',
          DOC_STATUS: this.field.DOC_STATUS ? this.field.DOC_STATUS.CODE : '',
          TASK: this.field.TASK,
          LISTING_NO: this.field.LISTING_NO,
          IS_YES_NO: this.field.IS_YES_NO,
