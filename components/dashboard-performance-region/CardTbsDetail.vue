<template>
  <div class="glass-card h-100 p-3 d-flex flex-column">
    <div class="d-flex align-items-center justify-content-center mb-1">
      <i class="fas fa-chart-bar text-success fa-2x mr-3"></i>
      <div>
        <h5 class="kpi-title mb-0">DETAIL PRODUKSI TBS</h5>
        <small class="text-muted text-uppercase font-weight-bold">(Detailed TBS Analysis)</small>
      </div>
    </div>
    <h6 class="text-center font-weight-bold mt-1 mb-0">PRODUKSI TBS KEBUN <small class="text-muted">(BEST TO WORST KEBUN)</small></h6>
    <div class="text-center text-muted small mb-1 font-weight-bold">Perkebunan Terbaik ke Terpuruk</div>
...
<script>
export default {
  name: 'DashboardPerformanceRegionCardTbsDetail',
  props: {
    tbsDetailData: {
      type: Array,
      default: () => null
  ...
  computed: {
    computedChartData() {
      ...
      if (!this.tbsDetailData || this.tbsDetailData.length === 0) {
      ...
      this.tbsDetailData.forEach(item => {
        if (item.plan > maxVal) maxVal = Number(item.plan);
        if (item.actual > maxVal) maxVal = Number(item.actual);

        categories.push({ label: item.company_code });
        
        planData.push({ 
          value: item.plan,
          color: "#1e8449", // dark green
          tooltext: `<b>${item.company_code} Target</b>: ${new Intl.NumberFormat('en-US', { maximumFractionDigits: 2 }).format(item.plan)} TON`
        });
        ...
        actualData.push({
          value: item.actual,
          color: actualColor,
          tooltext: `<b>${item.company_code} Aktual</b>: ${new Intl.NumberFormat('en-US', { maximumFractionDigits: 2 }).format(item.actual)} TON<br>Persentase: ${item.percentage}%`
        });
      });
