  ...
  computed: {
    ...
    computedChartData() {
      ...
      if (this.tbsData && this.tbsData.trend) {
        ...
        for (let i = this.tbsData.trend.length - 1; i >= 0; i--) {
          if (Number(this.tbsData.trend[i].total) > 0) {
            lastValidIndex = i;
            break;
          }
        }
        ...
        trendData = this.tbsData.trend.map((item, index) => {
          ...
          return {
            ...
            tooltext: `<b>${monthLabel}</b>: ${new Intl.NumberFormat('en-US', { maximumFractionDigits: 2 }).format(item.total)} TON`
          };
        });
      }
