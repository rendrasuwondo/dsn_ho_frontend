  ...
  computed: {
    ...
    computedChartData() {
      ...
      if (this.oekData && this.oekData.trend) {
        ...
        for (let i = this.oekData.trend.length - 1; i >= 0; i--) {
          if (Number(this.oekData.trend[i].total) > 0) {
            lastValidIndex = i;
            break;
          }
        }
        ...
        trendData = this.oekData.trend.map((item, index) => {
          ...
          return {
            ...
            tooltext: `<b>${monthLabel}</b>: ${new Intl.NumberFormat('en-US', { maximumFractionDigits: 2 }).format(item.total)}%`
          };
        });
      }
