      ...
      <div class="card card-outline card-info">
        <div class="card-header">
          <h3 class="card-title">
            ...
            <b>Data Approve Sampling Grading Internal</b>
          </h3>
        ...
        <div class="card-body">
          ...
          >
            ...
            <template #cell(qty_garbage)="row">
              <span>
                {{ formatToTwoDecimals(row.item.qty_garbage) }}
              </span>
            </template>
            <template #cell(percentage_garbage)="row">
              <span>
                {{ formatToTwoDecimals(row.item.percentage_garbage) }}
              </span>
            </template>
          </b-table>
          <button
  ...
  data() {
    ...
    return {
      ...
      fields: [
        ...
        { key: 'var_qty', label: '', tdClass: 'text-right' },
        {
          key: 'percentage_qty',
          label: '',
          formatter: this.formatToThousand,
      ...
      try {
        ...
        this.pt_id = query.company_code_plantation
          ? query.company_code_plantation
              .split(',')
              .filter(Boolean)
              .map((code) => ({ company_code_plantation: code }))
          : []
